...
export default function EditHabit() {
  ...
  async function handleSave() {
    if (!name.trim()) { Alert.alert('Erro', t('habit_name_placeholder')); return }
    setLoading(true)
    ...
    if (error) {
      ...
      if (enableReminder) {
        await scheduleHabitReminder(id, name.trim(), selectedIcon, reminderHour, reminderMinute, reminderFrequency, reminderEndHour)
      } else {
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={styles.header}>
        ...
        <Text style={[styles.title, { color: colors.text }]}>{t('edit_habit')}</Text>
        <View style={{ width: 24 }} />
      </View>
      ...
      <TouchableOpacity
        ...
        disabled={loading}
      >
        <Text style={styles.buttonText}>{loading ? t('saving') : t('save_changes')}</Text>
      </TouchableOpacity>
